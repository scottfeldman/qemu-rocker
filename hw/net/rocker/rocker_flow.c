...
static int of_dpa_cmd_add(struct flow_world *fw, uint64_t cookie,
                          struct rocker_tlv **tlvs)
{
    ...
    if (tlvs[ROCKER_TLV_OF_DPA_IG_PORT] && tbl == FLOW_TABLE_INGRESS_PORT)
        nest = tlvs[ROCKER_TLV_OF_DPA_IG_PORT];
    else if (tlvs[ROCKER_TLV_OF_DPA_VLAN] && tbl == FLOW_TABLE_VLAN)
    ...
    else if (tlvs[ROCKER_TLV_OF_DPA_BRIDGING] && tbl == FLOW_TABLE_BRIDGING)
    ...
    else if (tlvs[ROCKER_TLV_OF_DPA_ACL] && tbl == FLOW_TABLE_ACL_POLICY)
...
static int of_dpa_cmd_mod(struct flow_world *fw, uint64_t cookie,
                          struct rocker_tlv **tlvs)
{
    struct flow *flow = flow_find(fw, cookie);
...
static int of_dpa_cmd_del(struct flow_world *fw, uint64_t cookie)
{
    struct flow *flow = flow_find(fw, cookie);
...
static int of_dpa_cmd_get_stats(struct flow_world *fw, uint64_t cookie,
                     ...
                     struct rocker_tlv *cmd_info_tlv)
{
    ...
    case ROCKER_TLV_CMD_TYPE_OF_DPA_ADD:
        return of_dpa_cmd_add(fw, cookie, tlvs);
    case ROCKER_TLV_CMD_TYPE_OF_DPA_MOD:
        return of_dpa_cmd_mod(fw, cookie, tlvs);
    case ROCKER_TLV_CMD_TYPE_OF_DPA_DEL:
        return of_dpa_cmd_del(fw, cookie);
    case ROCKER_TLV_CMD_TYPE_OF_DPA_GET_STATS:
        return of_dpa_cmd_get_stats(fw, cookie, info, buf);
    }

    return -EINVAL;
}

static void of_dpa_default_bridging(struct flow_world *fw)
{
    struct flow *flow;
    struct group *group;
...
static void of_dpa_default_vlan(struct flow_world *fw)
{
    struct flow *flow;
...
static void of_dpa_default_ig_port(struct flow_world *fw)
{
    struct flow *flow;
...
static int of_dpa_world_init(struct world *world)
{
    ...
    of_dpa_default_ig_port(fw);
    of_dpa_default_vlan(fw);
    of_dpa_default_bridging(fw);
